...
export async function parseResumeWithOpenAI(file: File, signal?: AbortSignal): Promise<ProviderParseResult> {
  ...
  return {
    parsed, provider: "openai", fileId: uploaded.id, responseId: result.id || "", durationMs: Date.now() - started,
    tokenUsage: result.usage && { inputTokens: result.usage.input_tokens, outputTokens: result.usage.output_tokens, totalTokens: result.usage.total_tokens },
  };
}
