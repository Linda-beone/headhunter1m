...
export function ProjectForm({ project, demo }: { project?: Project; demo: boolean }) {
  ...
  const [form, setForm] = useState({ client_name: project?.client_name || "", job_title: project?.job_title || "", location: project?.location || "", salary_min: rangeValue(project?.salary_range, 0), salary_max: rangeValue(project?.salary_range, 1), experience_min: project?.experience_min?.toString() || "", experience_max: project?.experience_max?.toString() || "", must_have: project?.must_have.join("、") || "", nice_to_have: project?.nice_to_have.join("、") || "", target_companies: project?.target_companies.join("、") || "", status: project?.status || "draft" });
  const set = (key: string, value: string) => setForm(old => ({ ...old, [key]: value }));
  async function save() {
    if (demo) return setMessage("演示模式不会写入数据；连接 Supabase 后即可保存。 ");
    const list = (value: string) => value.split(/[、,，]/).map(s => s.trim()).filter(Boolean);
    const body = { client_name: form.client_name, job_title: form.job_title, location: form.location, salary_range: form.salary_min && form.salary_max ? `[${form.salary_min},${form.salary_max}]` : null, experience_min: Number(form.experience_min) || null, experience_max: Number(form.experience_max) || null, must_have: list(form.must_have), nice_to_have: list(form.nice_to_have), target_companies: list(form.target_companies), status: form.status };
